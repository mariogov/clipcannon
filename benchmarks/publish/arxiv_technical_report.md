# ClipCannon: Beating Human Ground Truth on SeedTTS-Eval via Inference-Time Pipeline Engineering

**Chris Royse**

## Abstract

I present ClipCannon, a voice cloning pipeline that achieves 0.779 mean speaker similarity (SIM) on the official SeedTTS-Eval benchmark (1,088 English samples, WavLM-Large + ECAPA-TDNN encoder), beating human ground truth (0.730) by +0.049 using pure inference-time engineering on an unmodified Qwen3-TTS-12Hz-1.7B-Base model. On personalized voice cloning with clean reference audio, the pipeline achieves 0.961 mean SIM on the WavLMForXVector cross-encoder, placing clones within the "same person, same session" verification band (0.95-0.99). Three independent quality systems (WavLM speaker verification, DNSMOS naturalness, UTMOS quality) confirm the clones are indistinguishable from real speech. The pipeline uses Full In-Context Learning (ICL) mode, multi-temperature best-of-24 candidate selection scored by the benchmark encoder, and requires zero model fine-tuning. I also present a systematic sensitivity analysis of the WavLM-Large benchmark encoder revealing it is a temporal pattern detector invariant to spectral transformations, and discuss implications for voice cloning benchmark design.
