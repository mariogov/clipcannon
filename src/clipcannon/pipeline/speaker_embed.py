...
def _load_audio(audio_path: Path) -> tuple[np.ndarray, int]:
    ...
    audio = data.astype(np.float32) / (32768.0 if data.dtype == np.int16 else 2147483648.0 if data.dtype == np.int32 else 1.0)
    if audio.ndim > 1:
...
def _detect_speech_vad(audio: np.ndarray, sample_rate: int) -> list[tuple[int, int]]:
    ...
    return [(int(ts["start"] / sample_rate * 1000), int(ts["end"] / sample_rate * 1000))
...
def _extract_wavlm_embeddings(
    ...
    speech_segments: list[tuple[int, int]], device: str,
    ...
    for start_ms, end_ms in speech_segments:
        ...
        with torch.no_grad():
            hidden = model(inputs["input_values"].to(device), output_hidden_states=True).last_hidden_state.squeeze(0).cpu().numpy()
...
def _insert_results(
    ...
    cluster_map: dict[int, list[int]], total_duration_ms: int,
) -> dict[str, int]:
    ...
    try:
        ...
        segments = conn.execute(
            "SELECT segment_id, start_ms, end_ms FROM transcript_segments WHERE project_id = ? ORDER BY start_ms",
            (project_id,)).fetchall()
        updated = 0
        for seg_row in segments:
            ...
            conn.execute("UPDATE transcript_segments SET speaker_id = ? WHERE segment_id = ?",
                         (best_speaker, seg_row[0]))
            updated += 1
        conn.execute(
            "UPDATE transcript_words SET speaker_id = ("
            "  SELECT ts.speaker_id FROM transcript_segments ts WHERE ts.segment_id = transcript_words.segment_id"
            ") WHERE segment_id IN (SELECT segment_id FROM transcript_segments WHERE project_id = ?)",
            (project_id,))
        conn.commit()
        counts["updated_segments"] = updated
    except Exception as exc:
        conn.rollback()
        raise PipelineError(f"Failed to insert speaker results: {exc}",
    ...
    try:
        vec_inserted = 0
        for idx, (start_ms, end_ms, emb) in enumerate(embeddings):
            sc = next((c for c, ii in cluster_map.items() if idx in ii), 0)
            emb_bytes = struct.pack(f"{len(emb)}f", *emb.tolist())
            try:
                vec_conn.execute(
                    "INSERT INTO vec_speakers (project_id, segment_text, timestamp_ms, speaker_id, speaker_embedding) VALUES (?, ?, ?, ?, ?)",
                    (project_id, "", start_ms, speaker_ids.get(sc, 1), emb_bytes))
                vec_inserted += 1
...
def _insert_fallback_speaker(db_path: Path, project_id: str) -> dict[str, int]:
    ...
    try:
        cursor = conn.execute(
            "INSERT INTO speakers (project_id, label, total_speaking_ms, speaking_pct) VALUES (?, ?, 0, 100.0)",
            (project_id, "speaker_0"))
        sid = cursor.lastrowid
        conn.execute("UPDATE transcript_segments SET speaker_id = ? WHERE project_id = ?", (sid, project_id))
        conn.execute(
            "UPDATE transcript_words SET speaker_id = ? WHERE segment_id IN "
            "(SELECT segment_id FROM transcript_segments WHERE project_id = ?)", (sid, project_id))
        conn.commit()
...
async def run_speaker_embed(
    ...
    try:
        ...
        if not speech_segments:
            ...
            return StageResult(success=True, operation=OPERATION,
        ...
        elapsed_ms = int((time.monotonic() - start_time) * 1000)
        input_sha = await asyncio.to_thread(sha256_file, audio_path)
        summary = json.dumps({"speakers": num_speakers, "speech_segments": len(speech_segments),
                               "embeddings": len(embeddings)}, sort_keys=True)
        record_id = record_provenance(
            db_path=db_path, project_id=project_id, operation=OPERATION, stage=STAGE,
            input_info=InputInfo(file_path=str(audio_path), sha256=input_sha,
                                 size_bytes=audio_path.stat().st_size),
            output_info=OutputInfo(sha256=sha256_string(summary), record_count=num_speakers),
            model_info=ModelInfo(name=backend_name, version=MODEL_ID,
            ...
            parent_record_id=None,
            description=(f"Speaker diarization ({backend_name}): {num_speakers} speakers, "
