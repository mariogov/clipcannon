...
def _compute_rms_envelope(
    audio: np.ndarray, sample_rate: int, hop_length: int = HOP_LENGTH,
...
def _detect_silence_gaps(
    rms: np.ndarray, time_ms: np.ndarray,
...
def _compute_spectral_flatness(
    audio: np.ndarray, sample_rate: int, window_ms: int = 2000,
        ...
        if flatness < SPECTRAL_FLATNESS_THRESHOLD and not in_music:
            ...
            if end_ms - music_start_ms >= MUSIC_MIN_DURATION_MS:
                music_sections.append({
                    "start_ms": int(max(0, music_start_ms)), "end_ms": int(end_ms),
                    "type": "music", "confidence": round(1.0 - float(flatness), 3),
                })
...
def _detect_beats_librosa(audio: np.ndarray, sample_rate: int) -> dict[str, object]:
    ...
    tempo_result, beat_frames = librosa.beat.beat_track(
        y=audio.astype(np.float32), sr=sample_rate, units="frames", hop_length=HOP_LENGTH,
    )
    tempo_bpm = float(tempo_result[0]) if isinstance(tempo_result, np.ndarray) and len(tempo_result) > 0 else float(tempo_result)
    beat_times = librosa.frames_to_time(beat_frames, sr=sample_rate, hop_length=HOP_LENGTH)
    beat_positions_ms = [int(t * 1000) for t in beat_times]
    has_music = len(beat_positions_ms) > 4 and tempo_bpm > 30
    return {
        "has_music": has_music, "source": "librosa",
...
def _detect_beats_scipy(audio: np.ndarray, sample_rate: int) -> dict[str, object]:
    ...
    if len(flux) < 4:
        return {"has_music": False, "source": "scipy_fallback", "tempo_bpm": None,
                "tempo_confidence": None, "beat_positions_ms": [],
                "downbeat_positions_ms": [], "beat_count": 0}
    peaks, _ = find_peaks(flux, height=np.mean(flux) + np.std(flux),
                          distance=int(sample_rate / hop * 0.3))
    ...
    return {
        "has_music": has_music, "source": "scipy_fallback",
...
def _insert_results(
    ...
    silence_gaps: list[dict[str, int]], avg_volume_db: float,
    dynamic_range_db: float, music_sections: list[dict[str, object]],
    ...
    try:
        counts: dict[str, int] = {}
        if silence_gaps:
            batch_insert(conn, "silence_gaps",
        ...
        execute(conn, "INSERT INTO acoustic (project_id, avg_volume_db, dynamic_range_db) VALUES (?, ?, ?)",
                (project_id, avg_volume_db, dynamic_range_db))
        counts["acoustic"] = 1
        if music_sections:
            batch_insert(conn, "music_sections",
                         ["project_id", "start_ms", "end_ms", "type", "confidence"],
                         [(project_id, int(s["start_ms"]), int(s["end_ms"]),
                           str(s.get("type", "music")), float(s.get("confidence", 0.5)))
        ...
        execute(conn,
                ...
                (project_id, bool(beats.get("has_music", False)), str(beats.get("source", "unknown")),
                 beats.get("tempo_bpm"), beats.get("tempo_confidence"),
                 json.dumps(beat_positions) if beat_positions else "[]",
                 json.dumps(downbeats) if downbeats else "[]", int(beats.get("beat_count", 0))))
        counts["beats"] = 1
        if beats.get("has_music") and beat_positions:
            ...
            if len(positions) >= 2:
                ...
                for idx in range(0, len(positions), ss):
                    ...
                    if len(chunk) >= 2:
                        section_rows.append((project_id, int(chunk[0]), int(chunk[-1]),
                                             beats.get("tempo_bpm"), "4/4"))
                if section_rows:
                    batch_insert(conn, "beat_sections",
        ...
        conn.rollback()
        raise PipelineError(f"Failed to insert acoustic results: {exc}",
                            stage_name=STAGE, operation=OPERATION) from exc
    finally:
        conn.close()


async def run_acoustic(
    project_id: str, db_path: Path, project_dir: Path, config: ClipCannonConfig,
) -> StageResult:
    ...
    try:
        ...
        logger.info("Audio loaded: %d samples, %d Hz, %.1fs",
        ...
        counts = await asyncio.to_thread(
            _insert_results, db_path, project_id, silence_gaps,
            avg_volume_db, dynamic_range_db, music_sections, beats)
        elapsed_ms = int((time.monotonic() - start_time) * 1000)
        input_sha = await asyncio.to_thread(sha256_file, audio_path)
        summary = json.dumps({"silence_gaps": counts.get("silence_gaps", 0),
                               "avg_volume_db": avg_volume_db, "dynamic_range_db": dynamic_range_db,
                               "music_sections": counts.get("music_sections", 0),
                               "has_music": beats.get("has_music", False),
                               "tempo_bpm": beats.get("tempo_bpm")}, sort_keys=True)
        record_id = record_provenance(
            db_path=db_path, project_id=project_id, operation=OPERATION, stage=STAGE,
            input_info=InputInfo(file_path=str(audio_path), sha256=input_sha,
                                 size_bytes=audio_path.stat().st_size),
            output_info=OutputInfo(sha256=sha256_string(summary), record_count=sum(counts.values())),
            model_info=ModelInfo(name="acoustic_analysis", version="1.0",
                                 parameters={"silence_threshold": SILENCE_RMS_THRESHOLD,
                                             "hop_length": HOP_LENGTH,
                                             "beat_source": str(beats.get("source", "unknown"))}),
            execution_info=ExecutionInfo(duration_ms=elapsed_ms),
            parent_record_id=None,
            description=(f"Acoustic: {counts.get('silence_gaps', 0)} gaps, vol={avg_volume_db}dB, "
