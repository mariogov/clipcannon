...
def _validate_project(project_id: str, required_status: str | None = "ready") -> dict[str, object] | None:
    ...
    if required_status is not None:
        ...
        if required_status == "created" and status != "created":
            return _error("INVALID_STATE", f"Project must be 'created' to ingest, current: {status}")
...
async def clipcannon_ingest(project_id: str, options: dict[str, object] | None = None) -> dict[str, object]:
    ...
    conn = get_connection(db, enable_vec=False, dict_rows=True)
    try:
        execute(conn, "UPDATE project SET status = 'analyzing', updated_at = datetime('now') WHERE project_id = ?", (project_id,))
        row = fetch_one(conn, "SELECT duration_ms FROM project WHERE project_id = ?", (project_id,))
        conn.commit()
    finally:
        conn.close()
    ...
    try:
        ...
        try:
            execute(err_conn, "UPDATE project SET status = 'error', updated_at = datetime('now') WHERE project_id = ?", (project_id,))
            err_conn.commit()
        finally:
            err_conn.close()
...
async def clipcannon_get_vud_summary(project_id: str) -> dict[str, object]:
    ...
    try:
        ...
        speakers = fetch_all(conn, "SELECT label, total_speaking_ms, speaking_pct FROM speakers WHERE project_id = ? ORDER BY speaking_pct DESC", (project_id,))

        tc_row = fetch_one(conn, "SELECT count(*) as cnt FROM topics WHERE project_id = ?", (project_id,))
        topic_count = int(tc_row["cnt"]) if tc_row else 0
        topic_preview = fetch_all(conn, "SELECT label, start_ms, end_ms, coherence_score FROM topics WHERE project_id = ? ORDER BY start_ms LIMIT 5", (project_id,))

        top_highlights = fetch_all(conn, "SELECT start_ms, end_ms, type, score, reason FROM highlights WHERE project_id = ? ORDER BY score DESC LIMIT 5", (project_id,))

        reaction_counts = fetch_all(conn, "SELECT type, count(*) as cnt FROM reactions WHERE project_id = ? GROUP BY type", (project_id,))

        beats_row = fetch_one(conn, "SELECT has_music, source, tempo_bpm, tempo_confidence, beat_count FROM beats WHERE project_id = ? LIMIT 1", (project_id,))

        safety_row = fetch_one(conn, "SELECT profanity_count, profanity_density, content_rating FROM content_safety WHERE project_id = ? LIMIT 1", (project_id,))

        energy_row = fetch_one(conn, "SELECT avg(energy) as avg_energy FROM emotion_curve WHERE project_id = ?", (project_id,))
        avg_energy = round(float(energy_row["avg_energy"]), 4) if (energy_row and energy_row.get("avg_energy") is not None) else None

        streams = fetch_all(conn, "SELECT stream_name, status, error_message FROM stream_status WHERE project_id = ? ORDER BY stream_name", (project_id,))
        failed_streams = [{"stream": str(s["stream_name"]), "error": str(s.get("error_message", ""))} for s in streams if str(s.get("status")) == "failed"]
        stream_summary = {
            "total": len(streams),
        ...
        prov_row = fetch_one(conn, "SELECT count(*) as cnt FROM provenance WHERE project_id = ?", (project_id,))
...
async def clipcannon_get_analytics(project_id: str, sections: list[str] | None = None) -> dict[str, object]:
    ...
    try:
        if "highlights" in sections:
            rows = fetch_all(conn, "SELECT * FROM highlights WHERE project_id = ? ORDER BY score DESC", (project_id,))
            result["highlights"] = [dict(r) for r in rows]

        if "scenes" in sections:
            tr = fetch_one(conn, "SELECT count(*) as cnt FROM scenes WHERE project_id = ?", (project_id,))
            total = int(tr["cnt"]) if tr else 0
            rows = fetch_all(conn, "SELECT * FROM scenes WHERE project_id = ? ORDER BY start_ms LIMIT ?", (project_id, _MAX_SCENES_PER_PAGE))
            result["scenes"] = {"total": total, "page_size": _MAX_SCENES_PER_PAGE, "data": [dict(r) for r in rows], "has_more": total > _MAX_SCENES_PER_PAGE}

        if "topics" in sections:
            rows = fetch_all(conn, "SELECT * FROM topics WHERE project_id = ? ORDER BY start_ms", (project_id,))
            result["topics"] = [dict(r) for r in rows]

        if "reactions" in sections:
            rows = fetch_all(conn, "SELECT * FROM reactions WHERE project_id = ? ORDER BY start_ms", (project_id,))
        ...
        if "beats" in sections:
            ...
            bs = fetch_all(conn, "SELECT * FROM beat_sections WHERE project_id = ? ORDER BY start_ms", (project_id,))
            result["beats"] = {"summary": dict(br) if br else None, "sections": [dict(r) for r in bs]}

        if "pacing" in sections:
            rows = fetch_all(conn, "SELECT * FROM pacing WHERE project_id = ? ORDER BY start_ms", (project_id,))
            result["pacing"] = [dict(r) for r in rows]

        if "silence_gaps" in sections:
            rows = fetch_all(conn, "SELECT * FROM silence_gaps WHERE project_id = ? ORDER BY start_ms", (project_id,))
...
async def clipcannon_get_transcript(project_id: str, start_ms: int = 0, end_ms: int | None = None) -> dict[str, object]:
    ...
    conn = get_connection(db, enable_vec=False, dict_rows=True)
    try:
        segments = fetch_all(conn, "SELECT segment_id, start_ms, end_ms, text, speaker_id, language, word_count FROM transcript_segments WHERE project_id = ? AND start_ms < ? AND end_ms > ? ORDER BY start_ms", (project_id, end_ms, start_ms))

        enriched: list[dict[str, object]] = []
        for seg in segments:
            sd = dict(seg)
            words = fetch_all(conn, "SELECT word, start_ms, end_ms, confidence, speaker_id FROM transcript_words WHERE segment_id = ? ORDER BY start_ms", (int(seg["segment_id"]),))
            sd["words"] = [dict(w) for w in words]
            enriched.append(sd)

        hm_row = fetch_one(conn, "SELECT count(*) as cnt FROM transcript_segments WHERE project_id = ? AND start_ms >= ?", (project_id, end_ms))
        has_more = int(hm_row["cnt"]) > 0 if hm_row else False

        dur_row = fetch_one(conn, "SELECT duration_ms FROM project WHERE project_id = ?", (project_id,))
