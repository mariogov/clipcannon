...
model with CUDA graphs.

Streaming mode: yields audio chunks as they are generated by the TTS
engine, achieving ~100-150ms TTFB on RTX 5090 (vs ~500ms non-streaming).
Each chunk is ~167ms of audio (2 codec tokens at 12Hz).
"""
...
class FastQwen3TTSService(TTSService):
    ...
    async def run_tts(
        ...
        """Convert text to audio frames via streaming synthesis.

        Called by Pipecat when a complete sentence arrives from the LLM.
        Streams audio chunks as they are generated by the TTS engine,
        yielding each chunk immediately for playback. First chunk arrives
        in ~100-150ms on RTX 5090 (vs ~500ms for full non-streaming).
        """
        ...
        logger.info("TTS streaming: '%s'", text[:80])
        ...
        try:
            ...
            async for chunk in self._adapter.synthesize_streaming(text):
                if t_first is None:
                    ...
                    ttfb = (t_first - t0) * 1000
                    logger.info("TTS TTFB: %.0fms", ttfb)

                chunk_int16 = (chunk * 32767).astype(np.int16)
                total_samples += len(chunk_int16)
                chunk_count += 1
            ...
            logger.info(
                "TTS done: %d chunks, %.2fs synth, %.2fs audio (RTF %.1f)",
                chunk_count, elapsed, audio_dur,
                audio_dur / max(elapsed, 0.01),
            )
